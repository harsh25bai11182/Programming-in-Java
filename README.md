...
- **Input Validation**: Validates user input through the utility layer.
...
`Main.java` is the **entry-point source file** of the application. It contains:
...
> **Important:** To start the application, run the **`Main` class**. In IntelliJ IDEA or VS Code, open `Main.java` and use the **Run** option.
...
4. Open **`Main.java`**.
5. Click **Run** or choose **Run `Main.main()`**.

`Main.java` is the entry-point source file, so the application starts from its `main()` method. IntelliJ handles the compilation and execution process automatically.
...
The project root contains the following source files and directories:
