...
5. **`Main`**
   - **Responsibilities:** Starts the application, displays the CLI menu, accepts user choices, and coordinates application workflow.
...
The project can also be opened in IntelliJ IDEA:
...
Compilation produces `.class` files in the project directories when `javac` is invoked without `-d`. These generated files are build output rather than source files.
...
└── Main.class              # Generated compiled class file
```

> **Note:** `.idea/` contains IntelliJ IDEA project configuration. `.class` files are generated during compilation and generally do not need to be committed to a Git repository.
